function [SpikeInds Kept_Spikes] = apply_dead_time2(SpikeInds,select_criterion,DT,Si,D)
% Function extracted from the detect spikes function, which has been now
% parsed.

% version 2 requires not a full data vector into which spikes are indices,
% but only peak values. It seems to make more sense, bu gotta keep an eye
% on it. 1/1/7
...
if ~isempty(DSi)
    while  ~Done
        ...
        switch select_criterion
            case 'takeL'  
            these_D_vals     = D(these_spike_inds);                
