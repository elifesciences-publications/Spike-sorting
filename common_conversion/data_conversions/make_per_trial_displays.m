%function make_per_trial_displays(SC,TimeVecCen,CutTimes,TbTa,event_times,event_trials,event_names,prev_event_times,prev_event_trials,prev_event_names,overlap,units,block_tag);
function make_per_trial_displays(sessionstr,spike_trials,spike_times,Tb,Ta,BinSize,CutTimes,event_times,event_trials,event_names,prev_event_times,prev_event_trials,prev_event_names,overlap,units,units_ord,block_tag);
...
for u = 1:length(spike_trials) % Over all units
    tu = units_ord(u);
    for i = 1:totntrials
        trialspikes = spike_times{tu}(spike_trials{tu} == i);
...
figure
fc = 1;
set(gcf,'name',[' trial display ' num2str(fc) ' ' sessionstr])
...
for i = 1:length(TrialMat)
    ...
    set(gca,'yticklabel',num2str(units(units_ord)'))
    xticks = [1:20:Nbins];
    ...
    for r = 1:length(these_names)
        ...
        thisx = 0.5 + Nbins * (these_times(r) + Tb)/(Tb+Ta);
