...
if ~isempty(DSi)
    while  ~Done
        ...
        if show_res
            plot(D(SpikeInds(these_spike_inds(1))-50:SpikeInds(these_spike_inds(end))+50))
            hold on
            plot(51+[0  diff(SpikeInds(these_spike_inds))],D(SpikeInds(these_spike_inds)),'r*')
            title(select_criterion)
        end
        ...
        if show_res
            plot(51+[0  diff(SpikeInds(these_spike_inds(good_one)))],D(SpikeInds(these_spike_inds(good_one))),'g*');
            hold off
            r = input('continue? Quit ?');
            if strcmp(r,'q')
                return
            end
        end
