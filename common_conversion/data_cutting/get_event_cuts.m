function [etimes, enames, etype, Events] =  get_Event_cuts(expdate,datadir,blocks);
...
Events = [];
for bi = 1:length(blocks)
    %%JFB edit 03nov11 %% for empty block
    if length(Event_time(bi))
